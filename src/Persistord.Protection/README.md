...
2. Annotate the secret with `[Protected]`
   ...
   ```
3. Call `ApplyProtection` last in `OnModelCreating`, after the module and
   entity configurations that create the annotated properties — it walks the
   model as already built, so a property configured afterwards is not seen:
...
reference to `Persistord.Protection` and a call to `ApplyProtection`, it is
stored as plaintext. The attribute alone changes nothing.

## The purpose string is fixed

Every protector `ApplyProtection` creates is derived from
`ProtectionPurposes.V1` (`"Persistord.Protection.v1"`). This is deliberate: a
protector derived from a different purpose cannot decrypt what this one
wrote, so the string can never change without orphaning every ciphertext
already in the database.

## One provider per application

EF Core caches the compiled model per context type. `ApplyProtection` bakes
the protector it was given into that cached model, so **the first
`IDataProtectionProvider` supplied to a given context type is the one every
subsequent instance of that context type uses**, for the lifetime of the
process — passing a different provider on a later call has no effect. The
supported arrangement is one `IDataProtectionProvider` per application,
registered as a singleton in DI and constructor-injected into the context.
...
- A key that was rotated away or deleted does not fail silently: reading a
  row encrypted under it surfaces as
