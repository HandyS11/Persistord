...
2. **`<section class="pd-transform">`** — three columns: `gateway event` → `.ToMessageEntity()` → `messages` row. Mark the id with `<span class="pd-snowflake">` on the left and `<span class="pd-row-id">` on the right. Per the colour rule, the left id is lavender (it came off the gateway) and the right is violet (it is in a column).
   ...
   **The true claim is quieter than it looks, and it is the one to make:** PostgreSQL and SQL Server have no native unsigned 64-bit type, so an unconverted `ulong` lands in a 20-digit fixed-point column — `numeric(20,0)` and `decimal(20,0)` respectively — not a `BIGINT`. The model still builds. (Two earlier drafts of this line claimed otherwise: that no provider has an unsigned 64-bit column at all — MySQL/MariaDB do — and that the model will not build. Both were false and both were caught only by probing. Verify before strengthening this sentence.) `DiscordDbContext` registers `UlongToLongConverter` in `ConfigureConventions`, so *every* `ulong` and `ulong?` in your model converts globally and you never annotate an id. The cast is `unchecked`, so it is bit-faithful across all 2^64 values — including the ones above 2^63 that no real-world id reaches today — a Steam64 id maxes out around 7.66e16, roughly 120x below 2^63, so do not cite one.

   So: use a realistic snowflake (`1547396186112000000` is a valid one for 2026), show it unchanged in a `BIGINT` column, and annotate underneath that Persistord is what converts the snowflake into a signed `BIGINT`, and that the same 64 bits go in and come back out — for every `ulong`, not just the ones that fit. Do not attribute the `BIGINT` to the provider; the provider's own answer is `numeric(20,0)`.
