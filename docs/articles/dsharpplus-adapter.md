...
The adapter adds `.To*Entity()` extension methods on DSharpPlus model classes. Import
the namespace and call the relevant mapper on the DSharpPlus object:
...
DSharpPlus models every entity as a single concrete class rather than splitting
gateway and REST variants, so there is one binding per entity — no separate overloads
needed. Threads map through `ToChannelEntity()` too: DSharpPlus carries the channel
kind in a property, so `DiscordThreadChannel` needs no method of its own.

### Why two methods take a guild id

`DiscordMember` and `DiscordRole` do not expose their guild id — it lives in an
internal field, and `DiscordMember.Guild` resolves through the client's guild cache
and throws for an uncached member. `MemberEntity.GuildId` (half of its composite
primary key) and `RoleEntity.GuildId` are not optional, so the caller passes the id,
which it always has at the point of mapping:
...
- **`UserEntity.GlobalName` stays `null`.** DSharpPlus 4.5.x has no equivalent of
  ...
  cache-resolved nickname-or-username fallback, a different concept, and it throws on
  an uncached member. If you need global names, set the property yourself.
- **`ChannelEntity.GuildId` is `0` for DM and group-DM channels,** which carry no
  guild id. Guild channels are unaffected.
- **`ToUserEntity()` throws on an uncached `DiscordMember`.** `DiscordMember` derives
  ...
  cache, so `member.ToUserEntity()` compiles and works against a live, cached client
  but throws `NullReferenceException` for a member the client has not cached. This is
  a DSharpPlus characteristic, not something the mapper works around.
...
Unrecognised kinds fall back to `Text` rather than throwing, so a channel kind
